...
- On `npm run build-data`:
  ...
  - It reads all JSON files from `repos/arcraiders-data/items/` and hideout definitions from `repos/arcraiders-data/hideout/`.
  - For each item it:
  ...
  - The processed result is written to:
    ...
    - `public/data/benches.json` – object mapping each craft bench’s internal `id` (from hideout data) to its localized display `name`, used by the React app to resolve craft bench IDs to readable names (e.g. the `craftBench` column).
  - Column definitions live in `public/columns.json` (manually maintained; the build script does not emit this file). The React app builds an item ID → name index from `items.json` at load time to resolve ID-based relationships (e.g. recipes) at render time.
