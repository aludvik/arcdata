...
function runTests() {
  ...
  let result = formatCellValue(row, "craftBench", {}, benches);
  ...
  result = formatCellValue(row, "craftBench", {}, benches);
  ...
  result = formatCellValue(row, "name", {}, benches);
  ...
  row = { tags: ["alpha", "beta"] };
  result = formatCellValue(row, "tags", {}, benches, {
    expanded: true,
  });
  assert.strictEqual(
    result.text,
